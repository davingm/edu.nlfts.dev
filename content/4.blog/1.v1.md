...
If you want help upgrading, you can join the [Discord Server](https://nlfts.dev/discord) and post in the `🔼・upgrade` channel or DM me on [Bluesky](https://bsky.app/profile/nlfts.dev) with a link to your project. I will do my best to answer questions / send PRs to help with the process.
...
edu.nlfts.dev now uses Tailwind v4, which is a major update to the Tailwind CSS framework. **You will need to update your `tailwind.css`** (follow the migration steps below). Apart from that, this update should be seamless for most users, but there are some breaking changes that you should be aware of:
...
/* If you are using edu.nlfts.dev in a monorepo, change the amount of .. to match the node_modules directory */
@source '../../node_modules/edu.nlfts.dev';
...
edu.nlfts.dev now supports i18n out of the box, including the following features:
...
::alert{icon="lucide:git-pull-request-arrow" to="https://github.com/nlfts/issues/125"}
...
You can now customize you font in `tailwind.config.js` file. edu.nlfts.dev uses `@nuxt/fonts` under the hood, so you can use any font you want.
...
- Navigation Menu viewport placement ([#23](https://github.com/nlfts/issues/23))
- 404 page size ([095135b](https://github.com/nlfts/commit/095135b))
- Aside font weight ([29ab698](https://github.com/nlfts/commit/29ab698))
- Add legacy: true to useClipboard ([7c56084](https://github.com/nlfts/commit/7c56084))
- Remove transpiles ([108fb35](https://github.com/nlfts/commit/108fb35))
